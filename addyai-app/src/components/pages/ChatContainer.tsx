...
export default function ChatContainer() {
  ...
  const [conversationHistory, setConversationHistory] = useState<Record<string, { id: number; headline: string; createdAt: string }[]> | null>(null);
  ...
  const handleSendMessage = useCallback(async (message: string) => {
    ...
    try {
      ...
      const botMessage = { message: data?.result ?? "Sorry, I wasn't able to find that answer.", isUserInput: false };
  ...
  const loadConversationById = useCallback(async (conversationId: number) => {
    try {
      const params = new URLSearchParams({
        user_id: String(userId),
        customer_id: customerId ?? "",
      ...
      <div className="flex flex-1 w-full">
        ...
        <div className="flex flex-col w-full">
          ...
          <main className="flex-1 flex flex-col items-center justify-between">
            ...
            <UserImportForm
              isLoading={isLoading}
              onMessageSubmitted={handleSendMessage}
            />
          </main>
        </div>
      </div>
